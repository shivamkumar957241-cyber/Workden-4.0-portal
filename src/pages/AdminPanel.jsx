...
export default function AdminPanel() {
  ...
  const { data: users = [] } = useFirebaseData('User');
  const { data: tasks = [] } = useFirebaseData('Task');
  const { data: proofs = [] } = useFirebaseData('Proof', '-created_date', 1000);
  const { data: withdrawals = [] } = useFirebaseData('WithdrawalRequest', '-requested_date');
  const { data: notifications = [] } = useFirebaseData('Notification', '-created_date', 500);
  const { data: walletTransactions = [] } = useFirebaseData('WalletTransaction', '-timestamp', 1000);
  const { data: globalSettings = [] } = useFirebaseData('GlobalSettings');
  const { data: subscriptionPayments = [] } = useFirebaseData('SubscriptionPayment', '-created_date');
  const { data: trainingVideos = [] } = useFirebaseData('TrainingVideo');
  const { data: recruiters = [] } = useFirebaseData('Recruiter');
  const { data: appUsers = [] } = useFirebaseData('AppUser', '-created_date', 1500);
  const { data: taskActivityLogs = [] } = useFirebaseData('TaskActivityLog', '-start_time', 1000);
  const { data: loginAttempts = [] } = useFirebaseData('LoginAttempt', '-login_time', 500);
  const { data: savedReplies = [] } = useFirebaseData('SavedRejectionReply', 'order');
  const { data: helpTickets = [] } = useFirebaseData('HelpTicket', '-created_date');
  const { data: userFeedbacks = [] } = useFirebaseData('UserFeedback', '-created_date');
  const { data: appreciationBonuses = [] } = useFirebaseData('AppreciationBonus', '-created_date');
  const { data: holidays = [] } = useFirebaseData('Holiday', 'holiday_date');
  const { data: referralPartners = [] } = useFirebaseData('ReferralPartner', '-created_date');
  const { data: callRequests = [] } = useFirebaseData('CallRequest', '-created_date', 500);
  useEffect(() => {
